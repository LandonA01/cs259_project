...
instr_trace_t* Emulator::execute(const Instr &instr, uint32_t wid) {
  ...
  if (rsrc3.type != RegType::None) fetch_registers(rs4_metadata, wid, 3, rsrc3);
  ...
  visit_var(op_type,
    ...
    ,[&](TcuType tcu_type) {
      ...
      case TcuType::SPMMA: {
        ...
        trace->data = trace_data;
        assert(warp.tmask.count() == num_threads);
        tensor_unit_->spmma(wid, tpuArgs.fmt_s, tpuArgs.fmt_d, tpuArgs.step_m, tpuArgs.step_n, rs1_data, rs2_data, rs3_data, rs4_metadata, rd_data, trace_data.get());
        rd_write = true;
      } break;
      default:
        std::abort();
